...
!--- Interfase ------------------------------------------------------------------!
  ...
  BLOCK_DIMENSION=9 	!Dimension of blocks: Impar para que sea simetrico
...
subroutine asignations_floquet
  use MultiDP_global
  implicit none
...
!--- Leads asignations -------------------------------------
    !Entro por el lead 9.
  do i=1,CHANNEL_NUMBER
    ener_lead(i)=0.d0 + hw*(dble( int((i+1)/2) )-dble(block_dimension+1)*0.5d0)
    hop_lead(i)=5.d0
    tun_lead(i)=1.d-3
  enddo
...
!--- Energias tight-binding
  do i=1,block_dimension
    do j=1,block_dimension
      if (i.eq.j) then
        ener(1,i,j)=dcmplx(ener_0+hw*(dble(i)-dble(block_dimension+1)*0.5d0),0.d0)
      elseif ((i.eq. j+1) .or. (j.eq. i+1)) then
        ener(1,i,j)=dcmplx(Vg,0.d0)
      else
...
end subroutine asignations_floquet
    ...
    if (decoherence .eq. 'Y') then
	 ...
	 l=1+int((dec_pos(i)-1)/BLOCK_DIMENSION)
       ...
       T_ef= T_LL - matmul(T_LD, matmul(T_DD,transpose(T_LD)) )
        ...
        write(77,1177)real(epsi(s)),(T_ef(9,j),j=2,channel_number)
        write(79,1177)real(epsi(s)),sum(T_ef(9,:))-T_ef(9,9)

   else !decoherence .ne. Y
	write(77,1177)real(epsi(s)),(T_LL(9,j),j=2,channel_number)
        write(79,1177)real(epsi(s)),sum(T_LL(9,:))-T_LL(9,9)
   endif 
